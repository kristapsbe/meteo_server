...
def get_city_response(city, add_last_no_skip, h_city, use_simple_warnings, add_city_coords):
    ...
    ret_val = {
        ...
        "daily_forecast": [{
            "time": f[1] if f[1][-4:] != "2300" else (datetime.datetime.strptime(f[1], "%Y%m%d%H%M") + datetime.timedelta(hours=1)).strftime("%Y%m%d%H%M"), # dealing with the clock getting turned forward
            "vals": f[2:]
        } for f in d_forecast],
